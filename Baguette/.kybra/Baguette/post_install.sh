...
exec 3>&1; output=$(KYBRA_VERSION=0.5.0 cargo run --manifest-path=.kybra/Baguette/kybra_post_install/Cargo.toml Baguette src/main.did 2>&1 1>&3 3>&-); exit_code=$?; exec 3>&-; if [ $exit_code -ne 0 ]; then echo "$output"; exit $exit_code; fi
